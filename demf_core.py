...
# Evaluation artifact filenames (used by Streamlit GUI)
...
def run_train_and_detect(cfg: DEMFConfig) -> Dict[str, pd.DataFrame]:
    ...
    # optional evaluation artifacts (won't break pipeline if labels are missing/mismatched)
    try:
        eval_out = save_evaluation_reports(scores=scores, features=user_day, cfg=cfg)
    except Exception:
        eval_out = None

    ret = {"events": events, "features": user_day, "scores": scores}
    if eval_out is not None:
        ret["evaluation_summary"] = eval_out.get("summary")
        ret["evaluation_by_day"] = eval_out.get("per_day")
        ret["evaluation_threshold_curve"] = eval_out.get("curve")
        ret["evaluation_scored"] = eval_out.get("scored")
    return ret
...
# -----------------------------
# Evaluation (optional): F1/precision/recall against labels CSV
# -----------------------------

_LABEL_COL_CANDIDATES = [
    "label", "is_anomaly", "anomaly", "ground_truth", "gt", "target", "y", "class"
]
_TIME_COL_CANDIDATES = [
    "day", "date", "timestamp", "time", "datetime"
]


def _eval_norm_cols(df: pd.DataFrame) -> pd.DataFrame:
    d = df.copy()
    d.columns = (
...
def load_labels(labels_path: Path, hash_salt: str) -> pd.DataFrame:
    """Load labels CSV and return canonical columns: user_hash, day, label (0/1)."""
    df = pd.read_csv(labels_path, encoding="utf-8-sig")
    df = _eval_norm_cols(df)

    if "user_hash" not in df.columns:
        if "user" in df.columns:
            df["user"] = df["user"].astype(str)
            df["user_hash"] = df["user"].apply(lambda x: sha256_hash(x, hash_salt))
        else:
            raise KeyError("Labels CSV must include 'user_hash' or 'user'.")
    ...
    for c in _TIME_COL_CANDIDATES:
        if c in df.columns:
            ...
            break
    if time_col is None:
        raise KeyError("Labels CSV must include a day/time column (e.g., 'day' or 'date').")

    df["day"] = pd.to_datetime(df[time_col], errors="coerce").dt.floor("D")
    ...
    for c in _LABEL_COL_CANDIDATES:
        if c in df.columns:
            ...
            break
    if lab_col is None:
        raise KeyError("Labels CSV must include a label column (e.g., 'label' or 'is_anomaly').")

    df["label"] = pd.to_numeric(df[lab_col], errors="coerce").fillna(0).astype(int)
    df["label"] = (df["label"] > 0).astype(int)

    df = df.groupby(["user_hash", "day"], as_index=False)["label"].max()
    return df


def evaluate_scores(scores: pd.DataFrame, features: pd.DataFrame, cfg: "DEMFConfig", threshold: float | None = None) -> dict:
    """
    Evaluate user-day predictions against a labels CSV (optional).
    Returns a dict with:
      - summary: overall metrics + confusion counts + thresholds
      - per_day: per-day precision/recall/F1
      - curve: precision/recall/F1 across thresholds in [0,1]
      - scored: joined rows (prediction + label)

    The labels CSV must contain:
      - user_hash OR user (will be hashed with cfg.hash_salt)
      - a time column (day/date/timestamp)
      - a label column (label/is_anomaly/ground_truth/...)
    """
    if cfg.labels_path is None:
        raise ValueError("cfg.labels_path is None")
    lp = Path(cfg.labels_path)
    if not lp.exists():
        raise FileNotFoundError(str(lp))

    labels = load_labels(lp, cfg.hash_salt)

    s = scores.copy()
    s["day"] = pd.to_datetime(s["day"], errors="coerce").dt.floor("D")

    f = features.copy()
    f["day"] = pd.to_datetime(f["day"], errors="coerce").dt.floor("D")

    # Bring canary_hit into scoring if present
    if "canary_hit" in f.columns:
        s = s.merge(f[["user_hash", "day", "canary_hit"]], on=["user_hash", "day"], how="left")
    else:
        s["canary_hit"] = False
    s["canary_hit"] = s["canary_hit"].fillna(False).astype(bool)

    # Determine current threshold (same logic as detection)
    current_threshold = None
    if threshold is not None:
        current_threshold = float(threshold)
    else:
        if "threshold" in s.columns and s["threshold"].notna().any():
            current_threshold = float(s["threshold"].dropna().iloc[0])
        else:
            current_threshold = float(np.quantile(s["final_score"].astype(float).to_numpy(), 1 - cfg.contamination))

    # Join labels and predictions
    m = s.merge(labels, on=["user_hash", "day"], how="inner")
    if m.empty:
        summary = {
            "status": "no_overlap",
            "labels_path": str(lp),
        ...
        return {
            "summary": summary,
            "per_day": pd.DataFrame(columns=["day", "precision", "recall", "f1", "support"]),
            "curve": pd.DataFrame(columns=["threshold", "precision", "recall", "f1"]),
            "scored": pd.DataFrame(),
        }
    ...
    # Prediction at current threshold (or canary)
    score_v = m["final_score"].astype(float).to_numpy()
    canary_v = m["canary_hit"].astype(bool).to_numpy()
    y_pred = ((score_v >= float(current_threshold)) | canary_v).astype(int)
    ...
    # Per-day metrics at current threshold
    per_day_rows = []
    for d, g in m.groupby("day"):
        yt = g["label"].astype(int).to_numpy()
        gv = g["final_score"].astype(float).to_numpy()
        cv = g["canary_hit"].astype(bool).to_numpy()
        yp = ((gv >= float(current_threshold)) | cv).astype(int)
        p, r, f1d, _ = precision_recall_fscore_support(yt, yp, average="binary", zero_division=0)
        per_day_rows.append(
            {"day": d, "precision": float(p), "recall": float(r), "f1": float(f1d), "support": int(len(g))}
        )
    per_day = pd.DataFrame(per_day_rows).sort_values("day")

    # Threshold curve across [0,1]
    ts = np.linspace(0, 1, 101)
    curve_rows = []
    for t in ts:
        yp = ((score_v >= float(t)) | canary_v).astype(int)
        p, r, f1t, _ = precision_recall_fscore_support(y_true, yp, average="binary", zero_division=0)
        curve_rows.append({"threshold": float(t), "precision": float(p), "recall": float(r), "f1": float(f1t)})
    curve = pd.DataFrame(curve_rows)

    # Best threshold by max F1 (first occurrence)
    best_threshold = None
    best_f1 = None
    if not curve.empty:
        idx_best = int(curve["f1"].values.argmax())
        best_threshold = float(curve.iloc[idx_best]["threshold"])
        best_f1 = float(curve.iloc[idx_best]["f1"])

    # Optional AUC metrics (only if both classes are present)
    roc_auc = None
    pr_auc = None
    try:
        if len(np.unique(y_true)) == 2:
            # Treat canary as a hard bump to 1.0 in the score for AUC ranking
            y_score = np.maximum(score_v, canary_v.astype(float))
    ...
    scored = m.copy()
    scored["pred_current"] = y_pred
    scored["current_threshold"] = float(current_threshold)

    summary = {
        "status": "ok",
        "labels_path": str(lp),
        "labeled_rows": int(len(m)),
    ...
    return {"summary": summary, "per_day": per_day, "curve": curve, "scored": scored}


def save_evaluation_reports(scores: pd.DataFrame, features: pd.DataFrame, cfg: "DEMFConfig") -> dict | None:
    """
    If cfg.labels_path is provided and exists, compute evaluation metrics and write artifacts into cfg.report_dir:
      - evaluation_summary.json
      - evaluation_by_day.csv
      - evaluation_threshold_curve.csv
      - evaluation_scored.csv

    Returns the evaluation dict, or None if labels are not provided.
    """
    if cfg.labels_path is None:
        return None
    lp = Path(cfg.labels_path)
    ...
    out = evaluate_scores(scores=scores, features=features, cfg=cfg, threshold=None)
    cfg.report_dir.mkdir(parents=True, exist_ok=True)

    (cfg.report_dir / EVAL_SUMMARY_FILE).write_text(json.dumps(out["summary"], indent=2), encoding="utf-8")
    out["per_day"].to_csv(cfg.report_dir / EVAL_BY_DAY_FILE, index=False)
    out["curve"].to_csv(cfg.report_dir / EVAL_THRESHOLD_FILE, index=False)
    out["scored"].to_csv(cfg.report_dir / EVAL_SCORED_FILE, index=False)
